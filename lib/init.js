...
class HtmlTidy {
   ...
   */
  activate() {
    this.configWatcher = atom.config.observe('atom-htmltidy', (value) => {
      this.isValid = false;
      this.settings = value;
    ...
    this.formatCommand = atom.commands.add('atom-text-editor', 'atom-htmltidy:format', () => {
      if (this.isValid) {
  ...
  _observeTextEditors() {
    if (this.settings.formatOnSave && this.isValid) {
      if (!this.textEditorObserver) {
        lodash.forEach(atom.workspace.getTextEditors(), (textEditor) => {
          this._watchTextEditor(textEditor);
        });
        this.textEditorObserver = atom.workspace.observeTextEditors(textEditor => {
          this._watchTextEditor(textEditor);
        });
      }
